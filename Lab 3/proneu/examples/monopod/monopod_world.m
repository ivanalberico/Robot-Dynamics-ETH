...
element(k).geometry.values = [0.1 0.05 5.0];
element(k).geometry.offsets.r = [0 -0.15 -2.5].';
element(k).geometry.offsets.C = eye(3);
...
world_name = 'MonopodWorld';

% Generate the world model object
worldmdl = WorldModel(element, @monopod_soft_contact_model, 'name', world_name);
