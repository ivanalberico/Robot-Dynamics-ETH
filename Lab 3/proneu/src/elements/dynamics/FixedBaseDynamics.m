...
classdef FixedBaseDynamics < handle
    
    % Fixed-Base System definitions
    properties
    ...
    methods
        ...
        function [] = getinversedynamics(obj)
            ...
            % Compute the total equations of motion as an implicit function
            tstart = tic;
            obj.ID = obj.M * obj.ddq + obj.b + obj.g - obj.tau_ext;
            tcomp = toc(tstart);
        ...
        function [ funcs ] = generatefunctions(obj, varargin)
            ...
            if isempty(variables)
                error('Variables have not been specified for FixedBodyDynamics functions.');
            end
            if isempty(parameters)
                error('Parameters have not been specified for FixedBodyDynamics functions.');
            end
            
            proneu_info('-->Generating numeric functions for the Fixed-Base dynamics. ');
...
%     symbols.ID = obj.ID;
% 
%     if ~isempty(symbols.ID)
%         symbols.ID      = subs(symbols.ID, parameters, values);
%     end
%     
%     if ~isempty(symbols.ID)
%         compute.ID      = matlabFunction(symbols.ID, 'vars', arguments);
%     end
% 
% end
